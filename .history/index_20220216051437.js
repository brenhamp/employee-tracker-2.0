...
async function addEmp(newEmpInfo) {
  ...
  console.log(
    `${newEmpInfo.firstName} ${newEmpInfo.lastName} added to the database.`
  );
}
...
async function updateEmp(newEmpInfo) {
  ...
  rows = await db.query(query, args);
  console.log(
    `${employee[0]} ${employee[1]}'s new role is ${newEmpInfo.role}.`
  );
}
...
async function main() {
  ...
  while (!exit) {
    ...
    switch (prompt.action) {
      ...
      case "changeRole": {
        const employee = await updateEmpInfo();
      ...
      default:
        console.log(`Unknown action ${prompt.action}`);
    }
  }
}
